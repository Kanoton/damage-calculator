...
(()=>{
 ...
 const characterChips=new Map();
 const selectedChips=document.querySelector('.selected-character-chips');
 function renderSelectedChips(){
  selectedChips.replaceChildren();
  if(!selectedCharacter)return;
  for(const chip of characterChips.get(selectedCharacter.id)||[]){
   const button=document.createElement('button');button.type='button';button.className='selected-chip';
   button.title=chip.name+'\n'+chip.effect+'\nクリックで削除';
   button.setAttribute('aria-label',chip.name+'：'+chip.effect+'。クリックで削除');
   const img=document.createElement('img');img.alt=chip.name;
   img.src='../images/chip_icon/'+encodeURIComponent(String(chip.images).trim());
   button.append(img);
   button.addEventListener('click',()=>{
    const owned=characterChips.get(selectedCharacter.id);
    owned.splice(owned.indexOf(chip),1);renderSelectedChips();
   });
   selectedChips.append(button);
  }
 }
 ...
 function renderImages(target,rows,folder,key){
  ...
  for(const row of sorted(rows)){
   ...
   const img=document.createElement('img');img.alt=row.name||row['名前']||'';img.loading='lazy';img.decoding='async';
   ...
   if(folder==='character_list'){
    ...
    const button=document.createElement('button');button.type='button';button.className='chip-select';
    button.title=row.name+'\n'+row.effect;
    button.setAttribute('aria-label',row.name+'：'+row.effect+'。選択中のキャラクターに追加');
    button.append(item);
    button.addEventListener('click',()=>{
     if(!selectedCharacter){chipStatus.textContent='先にキャラクターを選択してください。';return;}
     chipStatus.textContent='';
     if(!characterChips.has(selectedCharacter.id))characterChips.set(selectedCharacter.id,[]);
     characterChips.get(selectedCharacter.id).push(row);renderSelectedChips();
    });
 ...
 function renderChips(){
  renderImages(document.getElementById('chip-image-list'),chips.filter(row=>String(row.category).trim()===category),'chip','image');
 ...
 wireTabs(root.querySelector('.character-subtabs'),tab=>{
  document.getElementById('character-list-view').hidden=tab.id!=='character-list-tab';
  document.getElementById('character-chip-view').hidden=tab.id!=='character-chip-tab';
 });
 ...
 async function load(kind,file,status){
  ...
  try{
   const response=await fetch('../csv/'+file,{cache:'no-cache'});if(!response.ok)throw Error(file);
   const rows=parseMapCSV(await response.text());
   if(rows.length&&!Object.hasOwn(rows[0],'id'))throw Error('Missing id');
   return rows;
  }catch(error){status.textContent='CSVを取得できないため、同梱データを表示しています。';return parseMapCSV(CHARACTER_CSV_SNAPSHOT[kind]);}
 }
 Promise.all([load('characters','character_stats.csv',listStatus),load('chips','chip_list.csv',chipStatus)]).then(([a,b])=>{
  characters=a;chips=b;renderImages(document.getElementById('character-image-list'),characters,'character_list','list_img');renderChips();
 });
})();
