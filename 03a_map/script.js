...
function gimmickCount(row){return gimmickCounts.get(gimmickKey(row))??0;}
function effectiveStat(stats,key){
 ...
 mapGimmicks().filter(r=>r.monster_id===stats.monster_id&&(!r['難易度']||r['難易度']===difficulty.value)).forEach(r=>{
  ...
  const floor=r[key+'下限'];if(floor!==undefined&&floor!=='')minimum=minimum===null?Number(floor):Math.max(minimum,Number(floor));
 });
 return minimum===null?value:Math.max(minimum,value);
}
function renderGimmicks(){
 ...
 unique.forEach(row=>{
  const button=document.createElement('button');button.type='button';button.dataset.gimmick=row.gimmick_id;button.textContent=row['表示名']+' '+gimmickCount(row);button.setAttribute('aria-label',row['表示名']+' '+gimmickCount(row)+'、左クリックで増加、右クリックで減少');
  button.title='左クリック：＋1 ／ 右クリック：−1';
  const change=delta=>{const current=gimmickCount(row),maximum=row['最大回数']===''||row['最大回数']===undefined?Infinity:Number(row['最大回数']);gimmickCounts.set(gimmickKey(row),Math.max(0,Math.min(maximum,current+delta)));render();};
