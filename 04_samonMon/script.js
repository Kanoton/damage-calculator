...
function applyDefeatGimmicks(rows,totals,counts,mapId,level,monsterId){
 ...
 const key=row.map_id+':'+row.gimmick_id,rawMax=String(row['最大回数']??'').trim(),maximum=rawMax===''?Infinity:Math.max(0,Number(rawMax));if(Number.isNaN(maximum))return;counts.set(key,Math.min(maximum,(counts.get(key)||0)+1));
 });
...
function rememberRoster(){rosterHistory.push(structuredClone({monsters:placedMonsters,buff:rosterBuff,counts:[...rosterCounts],selected:selectedPlacedId,next:nextPlacedId,context:rosterContext,missions:[...missionCounters],events:[...executedEvents],defeats:[...defeatTotals]}));}
...
function clearRoster(){defeatTotals.clear();executedEvents.clear();placedMonsters.length=0;selectedPlacedId=null;rosterBuff.attack=0;rosterBuff.defense=0;rosterCounts.clear();rosterNotice.textContent='';}
