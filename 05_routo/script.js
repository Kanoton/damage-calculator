...
function groupMapEvents(events){
 const groups=new Map();for(const row of events){const progress=String(row['進捗']??'').trim(),route=String(row.route_id||'').trim(),key=JSON.stringify([route,progress]);if(!groups.has(key))groups.set(key,{進捗:progress,route_id:route,ID:row.ID,rows:[],内容:[]});const group=groups.get(key);group.rows.push(row);group['内容'].push(row['内容']);}
...
function missionCounterKey(row,level){return JSON.stringify([row.map_id,level,String(row.route_id||'').trim(),row.ID]);}

function decrementMonsterMissions(missions,counters,mapId,monsterId,level){
 missions.filter(row=>row.map_id===mapId&&routeLevelMatches(row,level)&&String(row.monster_id||'').split('|').map(id=>id.trim()).filter(Boolean).includes(monsterId)).forEach(row=>{
...
function renderMapInformation(){
 ...
 const missions=routeRows(data.missions).sort(byId);
